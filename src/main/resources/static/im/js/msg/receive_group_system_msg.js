...
function onKickedGroupNotify(notify) {
    //重新加载学生
    var isMySelfGroup = false;
    initStudentGroupInfo();
    studentListAll.forEach(function(index){
        if(index.group_id.search(notify.GroupId) > -1){
            isMySelfGroup = true;
        }
    })
    ...
    if(true){
        ...
        //刷新我的群组列表
        //getJoinedGroupListHigh(getGroupsCallbackOK);
        console.log("执行 被踢出群  回调：" + JSON.stringify(notify));
...
function onQuitGroupNotify(notify) {
    ...
    console.log("执行 主动退群  回调： " + JSON.stringify(notify));
    deleteSessDiv(webim.SESSION_TYPE.GROUP, notify.GroupId); //在最近的联系人列表删除可能存在的群会话
...
function initGetMyGroupSystemMsgs(data) {
    console.log("initGetMyGroupSystemMsgs : "+data);
