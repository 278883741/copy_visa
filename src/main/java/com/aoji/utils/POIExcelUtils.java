 ...
 * version: 小希 v1.0
 * author: yanjun
 * email: yanjun@aoji.cn
 * DateTime: 2017-12-27-14:17
 */
public class POIExcelUtils {
    ...
    public POIExcelUtils(Workbook workboook) {
        this.workbook = workboook;
    }

    public POIExcelUtils(InputStream is) throws FileNotFoundException, IOException {
        workbook = new XSSFWorkbook(is);
    }

    public POIExcelUtils(InputStream is, String version) throws FileNotFoundException, IOException {
    ...
    public POIExcelUtils(InputStream is, int size) throws FileNotFoundException, IOException {
    ...
    public boolean write(int sheetIx, List<List<String>> rowData, int startRow) throws IOException {
        ...
        if (getRowCount(sheetIx) > 0) {
            // 如果小于等于0，则一行都不存在
            sheet.shiftRows(startRow, getRowCount(sheetIx), rowData.size());
