...
/**
 * Description: Author: liubaocheng Create: 2017-03-28 17:01
 **/
public class ExcelUtils {
     ...
     * @see
     */
    public static <T> void listToExcelAddition(List<T> list, LinkedHashMap<String, String> fieldMap, String sheetName,
                                               int sheetSize, HttpServletResponse response, ExcelWriteInterceptor inter, StringParser parser)
            throws Exception {

        /*
         * if(CollectionUtils.isEmpty(list)){ throw new
         * ExcelException("数据源中没有任何数据"); }
         */
     ...
     */
    public static <T> void listToExcelAddition(List<T> list, LinkedHashMap<String, String> fieldMap, String sheetName,
                                               int sheetSize, HttpServletResponse response, ExcelWriteInterceptor inter) throws Exception {
    ...
    private static <E> void fillSheet(WritableSheet sheet, List<E> list, LinkedHashMap<String, String> fieldMap,
                                      int firstIndex, int lastIndex, int beginRow) throws Exception {
     ...
     */
    public static <T> void listToExcel(List<T> list, LinkedHashMap<String, String> fieldMap, String sheetName,
                                       int sheetSize, HttpServletResponse response) throws UnsupportedEncodingException {
        // 文件名默认设置为当前时间：年月日时分秒
        String fileName = DateFormatUtils.format(new Date(), "yyyyMMddHHmmss");
        // 设置文件名称
        String sname = new String(sheetName.getBytes("GBK"), "ISO-8859-1");
        // 设置response头信息
        response.reset();
        response.setContentType("application/vnd.ms-excel");
        response.setHeader("Content-disposition", "attachment;filename=" + sname + fileName + ".xls");
    ...
    public static <T> List<T> excelToList(InputStream is, String sheetName, Class<T> entityClass,
        ...
        try {
            ...
            if (sheet == null) {
                throw new RuntimeException("请检查sheet名称是否与提供的模板一致！");
            }
            ...
            for (int i = 1; i < realRows; i++) {
                ...
                for (int j = 0; j < length; j++) {
                    ...
                    Cell sameCell = sheet.findCell(currentContent, currentCell.getColumn(), currentCell.getRow() + 1,
                            currentCell.getColumn(), uniqueCells[j][realRows - 1].getRow(), true);
                    if (sameCell != null) {
                        nullCols++;
                    }
                }
     ...
     */
    private static <E> void fillSheet(WritableSheet sheet, List<E> list, LinkedHashMap<String, String> fieldMap,
                                      int firstIndex, int lastIndex, int beginRow, StringParser parser) throws Exception {
        // 定义存放英文字段名和中文字段名的数组
